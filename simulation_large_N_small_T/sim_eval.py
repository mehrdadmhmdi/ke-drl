    ...
    *,
    ...
    if metric not in df_all.columns:
        raise KeyError(
            ...
            f"Available columns include: {', '.join(map(str, df_all.columns[:25]))}"
        )
    ...
    values = pd.to_numeric(df_all[metric], errors="coerce")
    ...
    selected_values = pd.to_numeric(df[metric], errors="coerce")
    ...
    if "benchmark_id" in df_all.columns:
        ...
        for bid, group in df_all.groupby("benchmark_id", dropna=False):
            group_values = pd.to_numeric(group[metric], errors="coerce")
            ...
            rows.append(
                {
                    ...
                    f"{metric}_median_selected": (
                        float(pd.to_numeric(kept[metric], errors="coerce").median()) if len(kept) else np.nan
                    ),
                }
            )
