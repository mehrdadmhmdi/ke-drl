...
if mean_embedding_basis_size is None:
    raise RuntimeError("simulation_small_N_large_T must set mean_embedding_basis.n_basis so B has shape L x m.")
expected_B_shape = (int(pre.get("X_basis", pre["X_train"]).shape[0]), as_int(P["num_grid_points"]))
if tuple(B_hat.shape) != expected_B_shape:
    raise RuntimeError(
        "simulation_small_N_large_T must parameterize B by the selected mean-embedding basis: "
