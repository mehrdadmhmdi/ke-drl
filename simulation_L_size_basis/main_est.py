...
def reduce_transition_bank(
    *,
    ...
    This is only a memory/runtime device for large data banks. The row
    dimension of B is controlled separately by mean_embedding_basis.

    By default the landmarks are chosen in the joint transition space
    (S,A,S',A'), not only in current X=(S,A). If this optional approximation is
    enabled, the selected row indices are then
    applied consistently to s0, a0, s1, a1, and r0. Query anchoring is kept in
    current X=(S,A) space because training/evaluation target points only specify
    current state-action coordinates.
    """
    ...
    meta: dict[str, Any] = {
        ...
        "feature_space": str(cfg.get("feature_space", "transition")),
    ...
    feature_space = str(cfg.get("feature_space", "transition")).strip().lower()
    current_cpu = torch.cat([s0, a0], dim=1).to(dtype=torch.float32)
    next_cpu = torch.cat([s1, a1], dim=1).to(dtype=torch.float32)
    if feature_space in {"current", "x", "sa", "s_a"}:
        X_cpu = current_cpu
        feature_label = "current(S,A)"
    elif feature_space in {"transition", "joint", "current_next", "x_xplus", "sa_snext_anext"}:
        X_cpu = torch.cat([current_cpu, next_cpu], dim=1)
        feature_label = "transition(S,A,S',A')"
    elif feature_space in {"transition_reward", "joint_reward", "current_next_reward"}:
        X_cpu = torch.cat([current_cpu, next_cpu, r0.to(dtype=torch.float32)], dim=1)
        feature_label = "transition_reward(S,A,S',A',R)"
    else:
        raise ValueError(
            f"Unknown transition_reduction.feature_space={feature_space!r}. "
            "Use transition, current, or transition_reward."
        )
    ...
    print(
        "[transition reduction] enabled: original rows={}, target rows={}, method={}, feature_space={}, device={}".format(
            original_n, n_basis, method, feature_label, reduction_device
        ),
        flush=True,
    )
    ...
    if as_bool(cfg.get("include_query_nearest", True)):
        # Training/evaluation targets are current state-action points. Even
        # when landmarks are selected in joint transition space, nearest-query
        # anchoring must be performed in current X=(S,A) coordinates.
        X_query = current_cpu.to(reduction_device)
        query_mean = X_query.mean(dim=0, keepdim=True)
        query_sd = X_query.std(dim=0, keepdim=True, unbiased=False).clamp_min(1e-6)
        X_query_work = (X_query - query_mean) / query_sd if standardize else X_query
        q_cpu = torch.cat(
            [
        ...
        if standardize:
            q = (q - query_mean) / query_sd
        query_idx = _nearest_rows_to_queries(X_query_work, q, batch_size=batch_size)
    ...
    torch.save(
        {
            ...
            "query_anchor_space": "current(S,A)",
        },
        out_path,
    )
...
if mean_embedding_basis_size is None:
    raise RuntimeError("simulation_L_size_basis must set mean_embedding_basis.n_basis so B has shape L x m.")
expected_B_shape = (int(pre.get("X_basis", pre["X_train"]).shape[0]), as_int(P["num_grid_points"]))
if tuple(B_hat.shape) != expected_B_shape:
    raise RuntimeError(
        "simulation_L_size_basis must parameterize B by the selected mean-embedding basis: "
        f"expected B shape {expected_B_shape} = L x m, got {tuple(B_hat.shape)}."
    )
