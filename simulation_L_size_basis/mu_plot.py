...
reporting_filter = P.get("reporting_filter") or P.get("diagnostic_filter")
df = export_metrics_tables(mu_dir=args.mu_dir, metrics_dir=args.metrics_dir, reporting_filter=reporting_filter)
filter_summary_path = Path(args.metrics_dir) / "diagnostic_filter_summary.csv"
if filter_summary_path.exists():
    print(f"Diagnostic-filter summary written to {filter_summary_path}")
plot_mu_summary(mu_dir=args.mu_dir, metrics_dir=args.metrics_dir, outdir=args.plots_dir)
rep_plot_count = 0
if not args.skip_replicate_plots:
    rep_plot_count = plot_all_replicate_mu_diagnostics(
        mu_dir=args.mu_dir,
        metrics_dir=args.metrics_dir,
        outdir=args.plots_dir,
    )
